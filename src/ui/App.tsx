...
export const App: React.FC<AppProps> = ({ options, config: initialConfig, version, firstRun }) => {
  ...
  const { sessions, visibleItems, selectedSession, selectedGroup, selectedIndex, selectNext, selectPrev, toggleExpand, refresh } =
  ...
  const nicknameInput = useTextInput(
    (value) => { if (selectedSession && value.trim()) { setNickname(selectedSession.sessionId, value.trim()); refresh(); } setInputMode('normal'); },
    () => setInputMode('normal'),
  );
  ...
  useEffect(() => { setActivityScroll(0); }, [selectedSession?.sessionId, selectedGroup?.key]);
  ...
  const switchPanel = useCallback((dir: 'next' | 'prev') => split.switchPanel(dir, setActivePanel), [split.switchPanel]);
  const clearSplitState = useCallback(() => { split.clearSplitState(); setActivePanel('sessions'); }, [split.clearSplitState]);
  ...
  useKeyHandler({
    kb, activePanel, splitMode: split.splitMode, inputMode,
    showSetup: setup.showSetup,
    showSettings: setup.showSettings || setup.showThemeMenu || setup.showThemePicker || setup.showTour,
    showDetail, leftShowDetail: split.leftShowDetail, rightShowDetail: split.rightShowDetail,
    confirmAction, selectedSession, selectedGroup, toggleExpand,
    leftSession: split.leftSession, rightSession: split.rightSession,
    leftScroll: split.leftScroll, rightScroll: split.rightScroll,
    leftFilter: split.leftFilter, rightFilter: split.rightFilter,
    filter, activityFilter, viewingArchive, archivedIds, updateInfo,
    ...
    exit, selectNext, selectPrev, refresh, switchPanel, clearSplitState,
    resetPanel: split.resetPanel, getActiveFilter, setActivePanel, setInputMode,
    setFilter, setActivityFilter,
    setLeftFilter: split.setLeftFilter, setRightFilter: split.setRightFilter,
    setShowDetail,
    setLeftShowDetail: split.setLeftShowDetail, setRightShowDetail: split.setRightShowDetail,
    setShowSettings: setup.setShowSettings, setViewingArchive,
    setSplitMode: split.setSplitMode,
    setLeftSession: split.setLeftSession, setRightSession: split.setRightSession,
    setLeftScroll: split.setLeftScroll, setRightScroll: split.setRightScroll,
    setActivityScroll, setConfirmAction, setUpdateStatus,
    nicknameInput, filterInput,
    ...
    onDelete: (sess) => setConfirmAction({
      ...
      onConfirm: () => { deleteSessionFiles(sess.outputFiles); clearNickname(sess.sessionId); if (archivedIds.has(sess.sessionId)) { unarchiveSession(sess.sessionId); refreshArchived(); } refresh(); setConfirmAction(null); },
    }),
    onUpdate: () => { setUpdateStatus('updating...'); installUpdate().then(() => setUpdateStatus(`updated to v${updateInfo?.latest} — restart to apply`)).catch(() => setUpdateStatus('update failed')); },
  });
  ...
  if (setup.showSetup) {
    ...
    if (steps.length === 0) { setup.setShowSetup(false); return null; }
    return <SetupModal steps={steps} onComplete={setup.handleSetupComplete} />;
  }
  if (setup.showThemePicker) return <ThemePickerModal onSelect={setup.handleThemePickerSelect} onSkip={setup.handleThemePickerSkip} onDismiss={setup.handleThemePickerDismiss} />;
  ...
  if (setup.showSettings) return <SettingsMenu config={setup.liveConfig} onClose={setup.handleSettingsClose} onOpenThemeMenu={setup.handleOpenThemeMenu} />;
  if (confirmAction) {
    return (
  ...
  const rightPanel = split.splitMode ? (
    <SplitPanel activePanel={activePanel} leftSession={split.leftSession} rightSession={split.rightSession} leftEvents={leftEvents} rightEvents={rightEvents} leftScroll={split.leftScroll} rightScroll={split.rightScroll} leftFilter={split.leftFilter} rightFilter={split.rightFilter} leftShowDetail={split.leftShowDetail} rightShowDetail={split.rightShowDetail} height={mainHeight} />
  ) : showDetail && selectedSession ? (
    <SessionDetail session={selectedSession} focused={activePanel === 'activity'} height={mainHeight} />
  ) : (
    <ActivityFeed events={events} sessionSlug={activitySlug} sessionId={selectedSession?.sessionId} isActive={selectedGroup ? selectedGroup.isActive : selectedSession ? selectedSession.pid !== null : undefined} focused={activePanel === 'activity'} height={mainHeight} scrollOffset={activityScroll} filter={activityFilter || undefined} merged={isMerged} mergedSessions={selectedGroup?.sessions} />
  );
  ...
  return (
    <Box flexDirection="column" height={termHeight}>
      ...
      <Box flexGrow={1} height={mainHeight}>
        <SessionList visibleItems={visibleItems} selectedIndex={selectedIndex} focused={activePanel === 'sessions'} height={mainHeight} filter={filter || undefined} viewingArchive={viewingArchive} totalSessions={sessions.length} />
        {rightPanel}
      </Box>
      {!options.noSecurity && <AlertBar alerts={alerts} />}
      {inputMode === 'nickname' && <Box paddingX={1}><Text color={colors.primary}>nickname: </Text><Text color={colors.bright}>{nicknameInput.value}</Text><Text color={colors.muted}>_</Text></Box>}
      {inputMode === 'filter' && <Box paddingX={1}><Text color={colors.muted}>{activePanel === 'sessions' ? 'sessions' : activePanel === 'left' ? 'left' : activePanel === 'right' ? 'right' : 'activity'}</Text><Text color={colors.primary}>/</Text><Text color={colors.bright}>{filterInput.value}</Text><Text color={colors.muted}>_</Text></Box>}
      {inputMode === 'normal' && <FooterBar keybindings={kb} updateStatus={updateStatus} viewingArchive={viewingArchive} splitMode={split.splitMode} />}
    </Box>
  );
};
